...
terminalInputElement.addEventListener('input', function (e) {
  ...
  const filtredCommandList = allCommandList.filter((command) =>
...
terminalInputElement.addEventListener('change', async function (e) {
  ...
  const isCommandInvalid = commandWithParams.length > 2
  const isDouble = !isCommandInvalid && command === 'double'
  if (isCommandInvalid) {
    if (command === 'double') {
      terminalMessage = `Too much params. Try '${command} ${param}'`;
    } else {
      terminalMessage = 'Command not exist. Too much params.';
    }
  } else if (isDouble) {
    if (isNumeric(param)) {
